...
You can color a specific side only:

| Class | Description |
|-------|-------------|
| `.border-t-{color}` | Top border |
| `.border-r-{color}` | Right border |
| `.border-b-{color}` | Bottom border |
| `.border-l-{color}` | Left border |
...
**Important:** The class `.rounded` without suffix doesn't exist — use `.rounded-base`.
